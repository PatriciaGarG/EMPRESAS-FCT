  ...
  <form v-if="company" @submit.prevent="updateDataCompany" class="w-[50vw]">
    ...
    >
      <option
        ...
        :key="cycle.id"
        :value="cycle.id"
        :disabled="company.cycle_id.includes(cycle.id)"
      >
        {{ cycle.name }}
      </option>
    </select>
...
<script lang="ts" setup>
  ...
  const validateTitles = () => {
    ...
    errorTitle.value.address = !/^[a-zA-ZÀ-ÿ0-9/\\\\\s]{1,40}$/.test(
      company.value.address
    );
    errorTitle.value.cif = !/^[ABCDEFGHJKLMNPQRSUVW][0-9]{7}[0-9A-J]$/.test(
      company.value.cif
    );
    errorTitle.value.phone = !/^[0-9]{9,15}$/.test(company.value.phone);
    errorTitle.value.email =
  ...
  const addCycle = async () => {
    ...
    if (!error && newItem) {
      ...
      if (relationError) {
        console.error(
          'Error al añadir la relación empresa-ciclo:',
          relationError
        );
  ...
  const removeCycle = async (cycleId: string) => {
    ...
    if (!error) {
      company.value.cycle_id = company.value.cycle_id.filter(
        (id: string) => id !== cycleId
      );
    } else {
